function compareAllModels(hAxis1,hAxis2,hAxis3)
...
if nargin == 0
    ...
    [EstMdlMacro,estParamsMacro,EstParamCovMacro] = cs.fitSSMMacroYieldCurve();
    figure;
    hAxis1 = subplot(3,1,1);
    hAxis2 = subplot(3,1,2);
    hAxis3 = subplot(3,1,3);
elseif nargin == 3
    L1 = load('TwoStepModel.mat');
    EstMdlVAR = L1.EstMdlVAR;
    L2 = load('SSMModel.mat');
...
%% Model #3
A = EstMdlMacro.A(1:3,1:3);
imagesc(hAxis3,1:3,1:3,A);
hAxis3.XTick = 1:3; 
hAxis3.YTick = 1:3;
hAxis3.XTickLabel = {'L(t-1)','S(t-1)','C(t-1)'};
hAxis3.YTickLabel = {'L(t)','S(t)','C(t)'};
for i = 1:3
    for j = 1:3
        text(hAxis3,i,j,num2str(A(i,j),'%10.2f'),'Color','white')
    end
end
colorbar(hAxis3)
title(hAxis3,'Macro Enhanced - State Space Model')